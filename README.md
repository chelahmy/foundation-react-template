...
Foundation is a css-based framework. And React is a javascript-based framework. Fusing them together will complete front-end baseline development frameworks, and will lead developers to a different level of possibilities. Foundation uses [jQuery](https://jquery.com) so it is a bonus. But use jquery with care so not to snap React virtual DOM advantage.
