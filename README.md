...
Foundation is a responsive css-based framework. And React is a UI javascript-based framework. Fusing them together will complete front-end baseline development template, and will lead developers to a different level of possibilities. Foundation uses [jQuery](https://jquery.com) so it is a bonus. But use jquery with care so not to snap React virtual DOM advantage.
